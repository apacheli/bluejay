...
export default () => (tree) => {
    visit(tree, "blockquote", (node) => {
        ...
        node.data = {
            hProperties: { class: `alert-${type}` },
        };
        node.children.unshift({
            data: {
                hProperties: { class: `alerts-text alert-${type}-text` },
                hChildren: [
                    {
                        tagName: "svg",
                        type: "element",
                        properties: { class: "alerts-icon", viewBox: "0 0 16 16" },
                        children: [{ tagName: "path", type: "element", properties: { d: paths[type] } }],
                    },
                    {
                        type: "text",
                        value: type[0].toUpperCase() + type.slice(1),
                    },
                ],
            },
        });
