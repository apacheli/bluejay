...
export default () => (tree) => {
	visit(tree, "blockquote", (node) => {
		...
		const alertId = match?.[1].toLowerCase();
			...
			hChildren: [
				{
					tagName: "svg",
					type: "element",
					properties: { class: "alert-icon", viewBox: "0 0 16 16", "aria-hidden": true },
					children: [{ tagName: "path", type: "element", properties: { d: alert.path } }],
				},
				{ tagName: "span", type: "element", children: [{ type: "text", value: alert.title }] },
			],
		};
		node.children.unshift({ data });
		node.data = {
			hProperties: { className: `alert alert-${alertId}` },
		};
	});
};
